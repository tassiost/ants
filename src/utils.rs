...
pub fn calc_weighted_midpoint(points: &[(f32, f32, f32)]) -> Vec2 {
    ...
    points.iter().for_each(|(p0, p1, p2)| {
        total_weight += p2;
        weighted_sum_x += *p0 * p2;
        weighted_sum_y += *p1 * p2;
    });
...
pub fn get_rand_unit_vec3<R: Rng>(rng: &mut R) -> Vec3 {
    vec3(rng.gen_range(-1.0..1.0), rng.gen_range(-1.0..1.0), 0.0).normalize()
}

pub fn get_rand_vec2<R: Rng>(rng: &mut R) -> Vec2 {
    vec2(rng.gen_range(-W..W), rng.gen_range(-H..H))
}

pub fn get_rand_unit_vec2<R: Rng>(rng: &mut R) -> Vec2 {
    let rand_vec3 = get_rand_unit_vec3(rng);
